...
COMENTÁRIO:
Função que a gente provavelmente não vai usar, parâmetros complicados.
"""
...
class Sumo:
    ...
    def run_time(self, speed, time):
        self.r_motor.run_time(speed, time, then=Stop.HOLD, wait=False)
        self.l_motor.run_time(speed, time, then=Stop.HOLD, wait=False)
        self.r1_motor.run_time(-speed, time, then=Stop.HOLD, wait=False) 
